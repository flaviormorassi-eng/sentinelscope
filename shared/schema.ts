...
export const rawEvents = pgTable(
  ...
  (table) => ({
    processedIdx: index("raw_events_processed_idx").on(
      table.processed,
      table.receivedAt,
    ),
...
export const normalizedEvents = pgTable(
  ...
  (table) => ({
    userTimestampIdx: index("normalized_events_user_timestamp_idx").on(
      table.userId,
      table.timestamp,
    ),
...
export const threatEvents = pgTable(
  "threat_events",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    normalizedEventId: varchar("normalized_event_id")
      .notNull()
      .references(() => normalizedEvents.id),
    userId: varchar("user_id")
      .notNull()
      .references(() => users.id),
  ...
  (table) => ({
    userCreatedIdx: index("threat_events_user_created_idx").on(
      table.userId,
      table.createdAt,
    ),
...
export const intelMatches = pgTable("intel_matches", {
  id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  normalizedEventId: varchar("normalized_event_id").references(
    () => normalizedEvents.id,
  ),
...
export const insertThreatDecisionSchema = createInsertSchema(
  threatDecisions,
).omit({
  id: true,
  timestamp: true,
...
export const SUBSCRIPTION_TIERS = {
  individual: {
    name: "Individual",
    price: 5,
    stripePriceId: "prod_TLMdMqbS4BdOak",
    features: [
  ...
  smb: {
    name: "Small Business",
    price: 49.99,
    stripePriceId: "prod_TLMevoXf0Bw4SK",
    features: [
  ...
  enterprise: {
    name: "Enterprise",
    price: 199.99,
    stripePriceId: "prod_TLMfu4LGEzmQ5O",
    features: [
